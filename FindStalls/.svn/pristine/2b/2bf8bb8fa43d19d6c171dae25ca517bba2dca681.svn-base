...
@interface OrderDetail4TableViewCell()
@property (nonatomic, strong) UIView* _zxBgView;
@property (nonatomic, strong) UIButton* _zxBtn;
@property (nonatomic, strong) UIButton* _shBtn;
...
@implementation OrderDetail4TableViewCell
...
- (void)makeUI{
    ...
    self._zxBgView = [MyController viewWithFrame:self.contentView.frame];
    self._zxBgView.backgroundColor = [MyController colorWithHexString:@"fafafa"];
    [self.contentView addSubview:self._zxBgView];
    
    [self._zxBgView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    self._shBtn = [MyController createButtonWithFrame:self._zxBgView.frame ImageName:@"" Target:self Action:@selector(shouhouClick) Title:@"申请售后"];
    [self._zxBgView addSubview:self._shBtn];
    
    [self._shBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(-10);
        make.bottom.mas_equalTo(self._zxBgView.mas_bottom).mas_offset(-8);
        make.height.mas_offset(29);
        make.width.mas_offset(30);
    }];
    
    
    self._zxBtn = [MyController createButtonWithFrame:self._zxBgView.frame ImageName:@"" Target:self Action:@selector(zixunClick) Title:@"在线咨询"];
    [self._zxBgView addSubview:self._zxBtn];
    
    [self._zxBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self._shBtn.mas_left).mas_offset(-10);
        make.bottom.mas_equalTo(self._shBtn);
        make.height.mas_equalTo(self._shBtn);
        make.width.mas_equalTo(self._shBtn);
    }];
    ...
    [self._pzjLable mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self._zxBgView.mas_bottom).mas_offset(14);
    }];
...
- (void)shouhouClick {
    [self.OrderDetail4TableViewCellDelegate sendBackShouhou];
}

- (void)zixunClick {
    [self.OrderDetail4TableViewCellDelegate sendBackZixun];
}
