...
- (void)configCellWithModel:(MemberIndex1Model *)model {
    NSArray* titleArr = [[NSArray alloc] initWithObjects:@"我的账户",@"我的认证",@"我的红包",@"我的积分", nil];
    NSArray* titleArr1 = [[NSArray alloc] initWithObjects:@"形状-56",@"形状-57",@"形状-5811",@"形状-64", nil];
    ...
    for (int i = 0; i < titleArr.count; i++) {
        ...
        //左上图标
        UIImageView* temIV = [MyController createImageViewWithFrame:temView.frame ImageName:titleArr1[i]];
        ...
        if (i != 1) {
            [MyController fuwenbenLabel:countL FontNumber:[UIFont systemFontOfSize:12] AndRange:NSMakeRange(countL.text.length - 1, 1) AndColor:[UIColor blackColor]];
        }
